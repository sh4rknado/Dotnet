...
namespace Cisco_Script.Model {
    ...
    class Cisco_Device {
        ...
        /* Private Attributs */
        private string nom, password_enable, password_vty, password_console, banniere, type;
        ...
        /* Simplification de l'écriture de fichier */

        // Get all Infos_Device into List of string
        public List<string> GetList_Device() 
        {
            List<string> listes = new List<string>
            {
                ...
                Password_vty,
                Banniere,
                Domain_lookup.ToString(),
                Switch_ipv6.ToString(),
                Route_ipv6.ToString()
            };
            foreach (string s in GetList_Interfaces()) listes.Add(s);
            foreach(string s in GetList_Routes()) listes.Add(s);
            foreach(string s in GetMac_Table()) listes.Add(s);
            listes.Add("# end Cisco Devices");
            return listes;
        }        

        // Get list of interfaces in string
        private List<string> GetList_Interfaces() 
        {
            List<string> listes = new List<string>();
            foreach (Network_Interface ni in Network_Interfaces)
            {      
                listes.Add("# Interfaces Elements");
                listes.Add(ni.Name);
                listes.Add(ni.Numero);
                listes.Add(ni.Description);
                listes.Add(ni.Ip);
                listes.Add(ni.Netmask);
                listes.Add(ni.Is_active.ToString());
                listes.Add(ni.Is_ipv6.ToString());
                listes.Add("# end Elements");
            }
            return listes;
        }

        //Get list of Network Route into string
        private List<string> GetList_Routes() 
        {
            List<string> listes = new List<string>();
            foreach (Network_Route nr in Network_Routes)
            {      
                listes.Add("# Routes Elements");
                listes.Add(nr.Ip);
                listes.Add(nr.Next_ip);
                listes.Add(nr.Netmask);
                listes.Add("# end Elements");
            }
            return listes;
        }

        // Get list of ARP table
        private List<string> GetMac_Table() 
        {
            List<string> listes = new List<string>();
            foreach (MacAddressTable mac in Mac_tables)
            {      
                listes.Add("# Mac_Table Elements");
                listes.Add(mac.MacAddress);
                listes.Add(mac.Vlan);
                listes.Add(mac.InterfaceName);
                listes.Add("# end Elements");
            }
            return listes;
        }
